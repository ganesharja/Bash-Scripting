...
if [ -z $1 ] || [ -z $2 ]  ; then 
    ...
    echo -e "\e[35m Ex Usage \e[0m \n\t\t $ bash launch-ec2.sh ganesh"
    exit 1
fi 

# AMI_ID="ami-0c1d144c8fdd8d690"
AMI_ID= "ami-00a9c8ee62d3f943b" #"$(aws ec2 describe-images --filters "Name=name,Values=DevOps-LabImage-CentOS7"| jq ".Images[].ImageId" | sed -e 's/"//g')" 
SG_ID=  "sg-079390db7818da096" # "$(aws ec2 describe-security-groups  --filters Name=group-name,Values=B55_Allow_all | jq '.SecurityGroups[].GroupId' | sed -e 's/"//g')"       # B55-Allow-all security group id
